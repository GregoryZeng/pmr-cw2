...
if __name__ == "__main__":
    ...
    fig, ax = plt.subplots()
    ax.scatter(x=data[0],y=data[1], s=20, marker='.', label='data')
    ...
    result = sm_pca.sampling(data={'N': 15, 'D': 3, 'K': 2, 'X': data.T,
                                   'mu_W': 0, 'alpha0': 1, 'beta0': 1},
                             iter=10000, chains=2, n_jobs=2)
    ...
    for i in range(25):
        ...
        rescaled = e_vec * np.sqrt(e_val)[None,:]
        u_pairs += [rescaled[:2,sorted_idx[-1]], rescaled[:2,sorted_idx[-2]]]
    u_pairs = np.array(u_pairs)

    ax.scatter(x=u_pairs[::2, 0], y=u_pairs[::2,1],
               s=20, marker='o', label='PPCA $u_1$')
    ax.scatter(x=u_pairs[1::2, 0], y=u_pairs[1::2, 1],
               s=20, marker='o', label='PPCA $u_2$')

    # **** standard PCA ****
    u_pairs = []

    e_val, e_vec = np.linalg.eig(data @ data.T / data.shape[1])
    sorted_idx = np.argsort(e_val)
    rescaled_vec = e_vec * np.sqrt(e_val)[None,:]
    ax.scatter(x=rescaled_vec[0,sorted_idx[-1]],
               y=rescaled_vec[1,sorted_idx[-1]],
               s=20, marker='x', label='PCA $u_1$')
    ax.scatter(x=rescaled_vec[0, sorted_idx[-2]],
               y=rescaled_vec[1, sorted_idx[-2]],
               s=20, marker='x', label='PCA $u_2$')
